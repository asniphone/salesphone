...
import { updateUser, updateUserAccess, resetUserPassword, deleteUser, type UserDetailPayload } from "@/actions/user";
import {
...
export function UserDetailClient({ user, userAccess }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [errorProfile, setErrorProfile] = useState<string | null>(null);

  // Access map — initialize from user's current access values
  const [accessMap, setAccessMap] = useState<Record<AccessPermissionKey, boolean>>(() => {
    const map = {} as Record<AccessPermissionKey, boolean>;
    for (const key of ALL_ACCESS_KEYS) {
      map[key] = (user as Record<string, unknown>)[key] === true;
    }
    return map;
  });
  ...
  function onSaveAccess() {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        toast.error(result.error ?? "Gagal memperbarui hak akses.");
      }
    });
  }
    ...
    <div className="flex flex-col gap-6 w-full">
      <div className="w-full gap-6 flex flex-col lg:flex-row">
        ...
        <div className="space-y-6 w-full md:w-1/2 lg:w-1/3">
          {/* Card Edit Profile — only if has update access and not editing SuperAdmin (unless self) */}
          {userAccess.accessUserUpdate && (!user.isSuperAdmin || userAccess.userId === user.id) && (
            <Card>
          ...
          {/* Card View Only — when no update access or SuperAdmin viewed by non-self */}
          {(!userAccess.accessUserUpdate || (user.isSuperAdmin && userAccess.userId !== user.id)) && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                ...
                {user.isSuperAdmin && (
                  <CardDescription className="text-amber-600">Super Admin — Tidak dapat diedit oleh user lain.</CardDescription>
                )}
              </CardHeader>
          ...
          {/* Card Reset Password — only SuperAdmin caller can reset */}
          {userAccess.isSuperAdmin && !user.isSuperAdmin && (
          ...
          {/* Card Delete User — need delete access, cannot delete SuperAdmin */}
          {userAccess.accessUserDelete && !user.isSuperAdmin && (
      ...
      {/* Access Permission Editor — only if can update and not editing SuperAdmin (unless self) */}
      {userAccess.accessUserUpdate && (!user.isSuperAdmin || userAccess.userId === user.id) && (
        <div className="space-y-3">
          ...
          {!user.isSuperAdmin && (
            <div className="flex justify-end">
              <Button onClick={onSaveAccess} disabled={isPending} size="sm">
                {isPending ? "Menyimpan..." : (
                  <>
                    <Save className="w-4 h-4 mr-2" /> Simpan Hak Akses
                  </>
                )}
              </Button>
            </div>
          ...
          {user.isSuperAdmin && (
            <p className="text-xs text-amber-600 text-center">Super Admin memiliki semua akses secara otomatis.</p>
          )}
        </div>
      )}
    </div>
